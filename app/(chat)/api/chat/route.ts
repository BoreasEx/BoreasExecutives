...
function getLastMessageByRole(
  ...
  return [...uiMessages].reverse().find((m) => m.role === role);
}
...
function getLastMessageTextByRole(
  ...
  return getMessageText(getLastMessageByRole(uiMessages, role));
}
...
function countMessagesByRole(
  ...
  return uiMessages.filter((m) => m.role === role).length;
}
...
type EndDecision = "proceed" | "pause" | "reject" | null;
...
function getConversationDecision(message: string): EndDecision {
  const text = normalizeForEndCheck(message);

  if (!text) return null;

  if (/\bstatus\s*:\s*proceed\b/i.test(text)) return "proceed";
  if (/\bstatus\s*:\s*pause\b/i.test(text)) return "pause";
  if (/\bstatus\s*:\s*reject\b/i.test(text)) return "reject";
  ...
  const rejectClosures = [
    "we will not proceed",
    "we cannot proceed",
    "cannot proceed without",
    "contract drafting cannot proceed",
  ...
  if (positiveClosures.some((phrase) => text.includes(phrase))) {
    return "proceed";
  }

  if (pauseClosures.some((phrase) => text.includes(phrase))) {
    return "pause";
  }

  if (rejectClosures.some((phrase) => text.includes(phrase))) {
    return "reject";
  }
...
function isConversationEnded(message: string): boolean {
  return getConversationDecision(message) !== null;
}
...
- If you issue a final decision, end with exactly one standalone line:
  STATUS: PROCEED
  or STATUS: PAUSE
  or STATUS: REJECT
...
function hasExistingCertification(uiMessages: ChatMessage[]): boolean {
  return uiMessages.some((message) => {
    ...
    return message.parts.some((part) => {
      ...
      return typedPart?.type === "data-certification";
    });
  });
}
...
Express serious doubt, suspend validation, and require documented justification and exact producer identification.`;
    ...
    const alreadyCertified = hasExistingCertification(uiMessages);
    ...
    const stream = createUIMessageStream({
      ...
      execute: async ({ writer: dataStream }) => {
        ...
        const canTriggerOnThisAssistant =
          !alreadyCertified &&
          currentStep === 5 &&
          userTurns >= 5 &&
          assistantTurns + 1 >= 4;

        console.log("BOREAS_TRIGGER_GATE", {
          chatId: id,
          currentStep,
          userTurns,
          assistantTurnsBeforeGeneration: assistantTurns,
          assistantTurnsIncludingCurrent: assistantTurns + 1,
          alreadyCertified,
          lastAssistantMessageText,
          previousAssistantEnded: isConversationEnded(lastAssistantMessageText),
          scores: boreasScores,
          canTriggerOnThisAssistant,
        });
...
${supplierContext}`.trim(),
          ...
          onFinish: async (event) => {
            ...
            const decision = getConversationDecision(generatedAssistantText);
            const shouldTriggerEvaluation =
              canTriggerOnThisAssistant && decision !== null;
            ...
            if (!shouldTriggerEvaluation || !decision) {
              return;
            }
